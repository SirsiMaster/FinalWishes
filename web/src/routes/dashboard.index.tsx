    ...
    return (
      <div className="flex items-center justify-center h-[60vh]">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[11px] font-semibold text-royal uppercase tracking-[0.2em]">Loading your estate...</span>
        </div>
      </div>
    );
  }
    ...
    <div className="max-w-[1400px] mx-auto px-8 py-6">
        ...
        <div className="relative flex flex-col md:flex-row md:items-center justify-between gap-8 z-10">
          <div className="flex-1">
            <div className="flex items-center gap-3 mb-3">
              ...
              <span className="text-[10px] font-black text-royal uppercase tracking-[0.25em]">Your AI Guide</span>
            </div>
            <h1 className="font-[family-name:var(--font-cinzel)] text-2xl font-black text-royal tracking-tight leading-tight mb-2 uppercase">
              {userName ? (
                <>Welcome back, <span className="text-sapphire">{userName}</span>.</>
              ) : 'Your Estate is Active.'}
            </h1>
            <p className="text-[15px] text-royal/60 leading-relaxed font-bold">
              {insightData?.insight || "Your estate is currently 88% complete. We recommend updating the 'Primary Residence' valuation to reach 90% completion."}
            </p>
          </div>
          <button className="px-8 py-4 bg-royal hover:bg-sapphire text-white font-black text-[11px] uppercase tracking-widest rounded-2xl shadow-[0_4px_16px_rgba(19,51,120,0.25)] hover:shadow-[0_8px_24px_rgba(15,82,186,0.3)] transition-all active:scale-[0.98] whitespace-nowrap border border-white/10">
            {insightData?.actionLabel || 'Take Next Step →'}
          </button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        ...
        <MetricCard
          ...
          title="Family & Heirs"
          subtitle="Designated recipients"
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <DashboardCard title="Estate Setup Progress" subtitle="Track your progress toward a complete estate plan">
          ...
          <DashboardCard title="Recent Activity" subtitle="Verified audit trail of estate changes">
            <div className="space-y-0 -mx-2">
              <ActivityRow icon="📄" title="Document uploaded" desc="Property deed added to your Vault" time="2 hours ago" />
              <ActivityRow icon="👤" title="Beneficiary added" desc="Primary heirs confirmed" time="Yesterday" />
              <ActivityRow icon="🎬" title="Memoir entry saved" desc="Video memoir recorded and saved" time="3 days ago" />
              <ActivityRow icon="🏠" title="Asset updated" desc="Primary residence valuation updated" time="1 week ago" />
            </div>
          </DashboardCard>
        </div>
        ...
        <div className="lg:col-span-1 space-y-8">
          {/* Quick Actions */}
          <DashboardCard title="Quick Actions" subtitle="Common tasks for managing your estate">
            <div className="grid grid-cols-2 gap-4">
              <ActionButton icon="📦" label="Add Asset" desc="Inventory" />
              <ActionButton icon="📎" label="Upload Doc" desc="Vault" />
              <ActionButton icon="👥" label="Add Heir" desc="Family" />
              <ActionButton icon="🎥" label="AI Guidance" desc="Help" />
            </div>
          </DashboardCard>

          {/* Security Status */}
          <DashboardCard title="Security Status" subtitle="Your account protection details">
            <div className="space-y-4">
              ...
              <StatusRow label="Two-Factor Auth" value={metadata?.mfaEnabled ? "Enabled" : "Available"} color="green" />
              <StatusRow label="Data Compliance" value="HIPAA / PCI DSS" color="royal" />
              <StatusRow label="Estate ID" value={`FW-${estateId.slice(-4).toUpperCase()}`} />
            </div>
          </DashboardCard>
