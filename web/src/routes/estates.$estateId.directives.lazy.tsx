...
export function EditorToolbar({ editor, className = '' }: { editor: Editor; className?: string }) {
  ...
  return (
    <div
      ...
      className={`flex items-center gap-1 p-3 bg-neutral-faint rounded-2xl border border-neutral-border ${className}`}
    >
      {items.map((item, i) => {
        ...
        return (
          <Button
            ...
            onClick={item.run}
            className={`w-9 h-9 rounded-xl ${active ? 'bg-[var(--royal)] text-white hover:bg-[var(--royal)]/90 hover:text-white' : 'text-ink-muted hover:bg-[var(--neutral-border)]'}`}
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {directives.map((d) => {
            ...
            return (
              <Card
                key={d.id}
                className="cursor-pointer rounded-3xl border-neutral-border p-0 hover:border-[var(--royal)]/20 hover:shadow-lg transition-all group"
                ...
                <CardContent className="p-8">
                  <div className="flex items-start justify-between mb-6">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h3 className="text-lg font-bold text-royal group-hover:text-[var(--royal)] transition-colors">{d.title}</h3>
                        <p className="text-[12px] text-ink-muted font-medium">{cfg.label}</p>
                      </div>
                    </div>
                  ...
                  {d.recipientName && (
                    <p className="text-[13px] text-ink mb-3">
                      <span className="text-ink-muted">To:</span> {d.recipientName}
                      {d.recipientRelationship && <span className="text-ink-muted"> ({d.recipientRelationship})</span>}
                    </p>
                  )}
                  {d.content && (
                    <p className="text-[13px] text-ink-muted line-clamp-3">
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto rounded-3xl p-10" showCloseButton={false}>
        <DialogHeader className="mb-6">
          <DialogTitle className="text-2xl font-[family-name:var(--font-cinzel)] font-bold text-royal">
            Create Directive
          </DialogTitle>
        ...
        <div className="space-y-3 mb-8">
          ...
          {DIRECTIVE_TYPES.map((t) => {
            ...
            return (
              <Card
                key={t.value}
                className={`cursor-pointer p-0 rounded-2xl border-2 transition-all ${selectedType === t.value ? 'border-[var(--royal)] bg-[var(--royal)]/5' : 'border-neutral-border hover:border-gold/40'}`}
              ...
              >
                <CardContent className="flex items-center gap-4 p-5">
                  ...
                  <div>
                    ...
                    <p className="text-[12px] text-ink-muted">{t.description}</p>
                  </div>
                </CardContent>
              </Card>
            )
        ...
        <div className="mb-6 space-y-2">
          ...
          <Input
            ...
            className="h-auto px-5 py-4 rounded-2xl border-neutral-border text-[14px] text-ink focus-visible:border-[var(--royal)] focus-visible:ring-[var(--royal)]/50"
          />
        </div>
        ...
        <DialogFooter className="flex-row justify-end gap-4 mt-8 pt-8 border-t border-neutral-border rounded-b-3xl bg-transparent -mx-10 -mb-10 px-10 pb-10">
          <Button
            variant="ghost"
            onClick={() => onOpenChange(false)}
            className="px-8 py-4 h-auto rounded-2xl text-[14px] font-bold text-ink-muted hover:bg-neutral-faint"
          >
            Cancel
          </Button>
          <Button
...
function DirectiveEditor({ directive, estateId, onBack }: { directive: Directive; estateId: string; onBack: () => void }) {
  ...
  const editor = useEditor({
    ...
    editorProps: {
      attributes: {
        class: 'prose prose-lg max-w-none min-h-[400px] focus:outline-none p-8 text-ink',
      },
    },
    onUpdate: ({ editor: ed }) => {
  ...
  return (
    <div className="max-w-[1000px] mx-auto p-12 bg-white min-h-screen font-[family-name:var(--font-inter)]">
      ...
      <div className="flex items-center justify-between mb-10">
        ...
        <div className="flex items-center gap-3">
          ...
          {directive.status !== 'finalized' && autoSaveStatus !== 'idle' && (
            <span className={`flex items-center gap-1.5 text-[11px] font-medium mr-1 ${autoSaveStatus === 'error' ? 'text-[#DC2626]' : 'text-ink-muted'}`}>
          ...
          {directive.status !== 'finalized' && (
            <>
              <Button
                ...
                className="px-5 py-2.5 h-auto rounded-xl text-[12px] font-bold uppercase tracking-wider bg-neutral-faint text-ink hover:bg-[var(--neutral-border)]"
              >
      ...
      {/* ── Title Section ── */}
      <div className="mb-10 pb-8 border-b border-neutral-border">
        ...
        <h1 className="text-4xl font-[family-name:var(--font-cinzel)] font-bold text-royal tracking-tight">{directive.title}</h1>
        {directive.recipientName && (
          <p className="text-[15px] text-ink-muted mt-3">
            To: <span className="text-ink font-medium">{directive.recipientName}</span>
        ...
        {directive.status !== 'finalized' && heirs.length > 0 && (
          <div className="mt-5 pt-5 border-t border-neutral-faint">
              ...
              <button
                ...
                className={`text-xs px-3 py-1.5 rounded-full border transition-all font-medium ${
                  visibleTo.length === 0
                    ...
                    : 'border-neutral-border text-ink-muted hover:border-gold/50'
                }`}
              >
                Everyone
              </button>
      ...
      {/* ── Editor ── */}
      <div className="rounded-3xl border border-neutral-border overflow-hidden bg-white min-h-[500px]">
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <div>
          <h2 className="text-lg font-bold text-royal">Illinois Legal Advance Directives</h2>
          ...
          return (
            <Card
              ...
              className={`rounded-2xl transition-all cursor-pointer ${
                isComplete ? 'border-green-200 bg-green-50/30' : 'border-neutral-border hover:border-[var(--gold)]/20'
              }`}
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1">
                      <h3 className={`font-semibold text-sm ${isComplete ? 'text-green-700' : 'text-royal'}`}>
                        {d.name}
                      </h3>
                      ...
                      {d.overriddenBy && (
                        <Badge variant="outline" className="text-[10px] border-neutral-border text-ink-muted">
                          Overridden by HCPOA
                        </Badge>
                      )}
                    </div>
                    <p className="text-xs text-ink/60">{d.description}</p>
                  </div>
                </div>

                {isExpanded && (
                  <div className="mt-4 pt-3 border-t border-neutral-border space-y-3">
                    <div className="text-xs space-y-1">
                      <p className="text-ink/50 font-medium">Requirements:</p>
                    ...
                    <div className="text-xs space-y-1">
                      <p className="text-ink/50 font-medium">Key points:</p>
                      <ul className="space-y-0.5">
                        {d.keyPoints.map((kp, i) => (
                          <li key={i} className="text-ink/70 pl-3 relative before:content-['·'] before:absolute before:left-0 before:text-[var(--gold)]">
                            {kp}
                          </li>
                        ))}
                      </ul>
