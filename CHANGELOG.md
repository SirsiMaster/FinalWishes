...
- **First-load white-screen now self-heals for ANY mount failure** (`web/src/lib/chunk-reload.ts`) — the existing `vite:preloadError`/`unhandledrejection` handlers only catch chunk/MIME failures, but a first-load bootstrap can also blank `#root` from a network transient or silent render throw with no chunk error (observed live during a WiFi outage: all assets 200, `#root` empty, recovered only on manual reload). Added a mount watchdog: 8s after `load`, if `#root` is still empty, reload once (session-guarded against loops; a genuinely-broken build degrades to a static blank, never a reload storm); if mounted, clear the guard. Refs: F8 follow-up; docs/verification/PERSONA_MATRIX.md.
