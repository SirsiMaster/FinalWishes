    ...
    <div className="space-y-10 pb-20 max-w-6xl mx-auto">
      ...
      <div className="grid grid-cols-2 gap-8 max-md:grid-cols-1">
        {estates.map((e) => {
          ...
          return (
            <div key={e.id} className={`bg-white p-10 rounded-[2.5rem] border ${isCurrent ? 'border-royal/40 shadow-2xl' : 'border-border-light shadow-sm'} hover:shadow-2xl hover:border-royal/20 transition-all group overflow-hidden relative active:scale-[0.98]`}>
              ...
              <div className={`w-16 h-16 ${isCurrent ? 'bg-royal text-white' : 'bg-royal-subtle text-royal'} rounded-2xl flex items-center justify-center mb-6 border border-royal/10 shadow-lg`}>
              ...
              <div className="flex gap-4">
                ...
                <button 
                  ...
                  className={`flex-1 ${isCurrent ? 'bg-black opacity-50 cursor-default' : 'bg-navy hover:bg-black'} text-white px-6 py-3 rounded-2xl text-[0.7rem] font-black uppercase tracking-widest transition-all shadow-lg`}
                >
                  {isCurrent ? 'Focus Active' : 'Switch Focus'}
                </button>
              </div>
            </div>
          );
