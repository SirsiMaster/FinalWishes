...
import { getMFAStatus, startTotpEnrollment, finalizeTotpEnrollment, unenrollTotp, type MFAEnrollmentState } from '../lib/mfa'
  ...
  return (
    <div className="max-w-[1000px] mx-auto space-y-10 pb-20 px-4">
      ...
      <div className="grid grid-cols-1 gap-12">
        ...
        <MFAEnrollmentSection user={user} mfaStatus={mfaStatus} isFiduciary={!!isFiduciary} />
        ...
        <section className="bg-white rounded-[3rem] border border-royal/10 overflow-hidden shadow-[0_2px_20px_rgba(19,51,120,0.03)] hover:shadow-[0_20px_50px_rgba(19,51,120,0.08)] hover:border-royal/20 transition-all group relative">
          ...
          <SettingsSection title="Security Settings">
            <SettingsItem label="Two-Factor Authentication" value={mfaStatus.enrolled ? "Active" : "Not Active"} type="status" />
...
function MFAEnrollmentSection({ user, mfaStatus, isFiduciary }: { user: any; mfaStatus: { enrolled: boolean; methods: string[] }; isFiduciary: boolean }) {
    ...
    <div className="flex items-center justify-between px-10 py-7 hover:bg-royal/[0.01] transition-all group cursor-pointer relative">
      ...
      <div className="flex items-center gap-4">
        {type === 'toggle' ? (
          ...
          <span className={`text-[9px] font-black uppercase tracking-[0.2em] px-4 py-1.5 rounded-xl shadow-sm ${
            value === 'Active' || value === 'Verified' 
              ? 'text-green-600 bg-green-50 border border-green-100' 
              : 'text-royal/40 bg-royal/[0.02] border border-royal/5'
          }`}>{value}</span>
        ) : (
