...
### Known / operational
- **SECURITY (HIGH, pre-existing, flagged for coordinated fix) — Soul Log private entries are readable by any estate member at the DB layer.** The Firestore rule `match /soul-log/{entryId}` uses `allow read: if canAccessEstate(estateId)` — so any accessor (heir/executor/trustee/legal/cpa) can read **all** entries, including the owner's `private` diary, directly via the SDK. The route's client-side filter (which matches the viewer's `displayName` against `taggedPeople`) only hides them in the UI and is itself weak (display-name collision/rename bypasses it; `taggedPeople` stores names, not UIDs). This contradicts ETHOS ("a locked diary") and Rule 26. **Not fixed solo** because the correct fix is a coordinated redesign that would break the feature if done half-way: (1) migrate `taggedPeople` to UIDs; (2) constrain non-owner soul-log queries to `where('visibility','in',['shared','sealed'])` + `array-contains` the viewer UID; (3) split the rule into `allow get` (owner/writer OR shared-tagged-to-uid OR delivered-sealed) and `allow list` (only when the query is so-constrained). Routed to codex-finalwishes. **Should be closed before GA** or accepted with a documented risk decision.
