...
export function SettlementGantt({ deadlines, dateOfDeath }: SettlementGanttProps) {
  ...
  return (
    <div className="w-full">
        ...
        >
          ...
          <Tooltip
            content={({ active, payload }) => {
              ...
              return (
                <div className="bg-white border border-[#142848]/10 rounded-xl px-4 py-3 shadow-lg text-sm">
                  <p className="font-semibold text-[#142848]">{d.name}</p>
                  <p className="text-[#142848]/60 text-xs mt-1">
                    Due: {d.dueDate}
                  </p>
                  <p className={`text-xs mt-0.5 font-medium ${d.overdue ? 'text-red-600' : 'text-[#142848]/50'}`}>
                    {d.overdue
      ...
      {/* Legend */}
      <div className="flex items-center justify-center gap-4 mt-2 text-[10px] font-medium uppercase tracking-wider text-[#142848]/50">
