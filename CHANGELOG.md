...
- **End-to-end verification sweep (every user process)** — drove the real app on live prod with a sanctioned test account (the `principal@finalwishes.app` password-set trick) and found/fixed several issues that unit tests + the web-only pre-push gate missed:
  ...
  - **The Shepherd conversational chat returned 500 in production** — `/api/v1/guidance/chat` failed with "Failed to generate response" because the Cloud Run service has **no AI provider key** (`NewShepherdAdvisor` (sirsi-ai/Claude) and the Genkit/Gemini fallback both need a key; the service env has Stripe/SendGrid/Vault/`GOOGLE_CLOUD_PROJECT` but no `SIRSI_AI`/`ANTHROPIC`/`GEMINI` key). The deterministic `/guidance/score` is unaffected. Made the chat handler **fail soft**: it still logs the error (ops visibility) but returns a warm, honest 200 fallback with suggested actions instead of a 500, so the Shepherd never feels broken. **Owner action still required**: provision an AI key as a Cloud Run secret + pin it in the deploy workflow (same pattern as `GOOGLE_CLOUD_PROJECT`) to restore full conversational guidance.
  - **E2E suite couldn't actually run the authenticated flows** — `web/e2e/helpers/auth.ts` navigated to `/?login=true` (bare), which TanStack Router parses as a boolean and does NOT open the login modal (the app emits `?login="true"`, JSON-quoted, via the `/login` route). Pointed the helper at `/login`, and added dismissal of the first-run `OwnerWelcome` takeover so specs reach the dashboard. (Remaining: the authenticated/life-first specs still assume a flat sidebar; the shipped sidebar is grouped/collapsible — spec modernization tracked separately.)
