...
## [Unreleased]
### Added
- **Google OAuth verification package for Google Photos import (ADR-045, FR-905)** (`web/src/routes/privacy.tsx`, `docs/GOOGLE-PHOTOS-VERIFICATION.md`) — prepares the OAuth app verification so the Heirloom Registry's "Import from Google Photos" works for ALL users, not just consent-screen test users. The privacy policy gains a dedicated **Google Photos** section: states the app uses the **Photos Picker API**, accesses **only the photos the user selects** in Google's own picker (read-only, never the whole library), stores them as the user's heirlooms (not sold / not for advertising / not shared beyond providing the feature), and carries the **verbatim Limited Use affirmation** with a link to the Google API Services User Data Policy. A new submission runbook (`docs/GOOGLE-PHOTOS-VERIFICATION.md`) gives the owner the exact consent-screen field values (app name, support email `support@sirsi.ai`, homepage/privacy URLs, authorized domains `finalwishes.app` + `finalwishes-prod.web.app`), copy-paste scope justification for `photospicker.mediaitems.readonly`, Google Search Console domain-verification steps, a 30–60s demo-video script (consent screen → grant → picker → heirloom appears), and a submission checklist noting the Picker scope is the lighter **sensitive** tier (standard verification, ~days–weeks) NOT the heavy restricted/CASA assessment (confirm tier in the Verification Center). No code/redeploy needed post-approval — the client id + Picker flow are already live.
...
### Changed
- **Google Photos import wired live (FR-905, ADR-045)** (`web/src/lib/google-photos-import.ts`) — set the project's Web OAuth client id (`860699311615-…apps.googleusercontent.com`, finalwishes-prod) as a committed env-overridable default (public, ships in the bundle like `firebaseConfig.apiKey`), so the CI production build enables the "Import from Google Photos" button on the Heirloom Registry. Requires the owner's OAuth consent-screen scope `photospicker.mediaitems.readonly` + Picker API (both done in console) to authorize at runtime.
...
### Fixed
- **Legal RAG corpus loaded to prod + schema 3072-dim index fix (CR-10, ADR-044)** (`api/internal/guidance/schema.sql`, `docs/legal-corpus/ENABLE-RAG.md`) — ran the real ingest into the new isolated `legal_rag` database on Cloud SQL `finalwishes-pii-vault`: **7 sources / 64 chunks** (3 IL, 1 MD, 3 MN), all embedded with `gemini-embedding-001` (3072-dim, 64/64 present); nearest-neighbour retrieval verified (IL statutes cluster, self-match 1.000). Fixed a schema bug — `schema.sql` tried to build an `ivfflat` index on `embedding`, but pgvector's ivfflat/hnsw cap at 2000 dims while the embedding is 3072-dim, so applying the schema errored; removed the ANN index (retrieval uses an exact `<=>` cosine scan, sub-ms at corpus scale) with a note on the `halfvec(3072)` path if the corpus ever needs ANN. **RAG stays DISABLED in prod** (`RAG_DATABASE_URL` unset on Cloud Run) so nothing is user-facing; `docs/legal-corpus/ENABLE-RAG.md` documents the legal-review gate + the one-command enable.
...
- **SECURITY — Soul Log private diary is no longer readable by other estate members** (`firestore.rules`, `firestore.indexes.json`, `web/src/routes/estates.$estateId.soul-log.lazy.tsx`) — closes the HIGH pre-existing gap where `allow read: if canAccessEstate` let any accessor read **all** entries (incl. the owner's `private`/`sealed` diary) via the SDK. Now: **owner/admin read everything (path unchanged); every other member reads ONLY `shared` entries** — rule = `isEstatePrincipal || isAdmin || (canAccessEstate && resource.data.visibility == 'shared')`. The soul-log route resolves the **estate-scoped** role and constrains the non-owner query to `where('visibility','==','shared')` (gated until the role is definitive so no unconstrained/denied read fires), with a new `(visibility, createdAt)` composite index. The owner's query + experience are untouched (blast radius limited to the non-owner view). **Residual (tracked for Codex):** per-recipient narrowing of *shared* entries is still a client-side `taggedPeople` (display-name) filter — a non-owner can read all *shared* entries, not just ones tagged to them; the UID migration for rule-level per-recipient enforcement remains. The critical *private*-entry exposure is closed. ETHOS "locked diary" / Rule 26.
- **Code-review hardening pass (correctness + security)** — an adversarial review of the session's persona-safe + CRUD changes surfaced real issues, now fixed:
  - **CRITICAL — Rules-of-Hooks crash:** `notifications.tsx` called `useState` *after* the `if (isLoading) return` early return — a hooks-order violation that corrupts state in prod (tests + typecheck can't catch it). Moved the hook above the conditional return.
