...
export function HeirWelcome({ estateId, onContinue }: HeirWelcomeProps) {
  ...
  // Fetch the estate owner's profile from Firestore
  const ownerPath = estate?.principalId ? `users/${estate.principalId}` : null
  const { data: ownerProfile } = useDocument<OwnerProfile>(ownerPath)
  ...
  const { data: allHeirlooms, loading: heirloomsLoading } = useHeirlooms(estateId)
  ...
  const ownerName = ownerProfile?.displayName || estate?.name || 'Your loved one'
  ...
  const dateSpan = useMemo(() => {
    ...
    const parts = [ownerProfile.birthDate, ownerProfile.deathDate].filter(Boolean)
    return parts.join(' \u2014 ')
  }, [ownerProfile])

  const hasPersonalMessages = personalCapsules.length > 0 || personalEntries.length > 0
...
export function shouldShowHeirWelcome(
  ...
  const isSettlement = estateStatus === 'in_settlement' || estateStatus === 'death_reported' || estateStatus === 'executor_confirmed'
  const isFirstVisit = typeof window !== 'undefined' && !localStorage.getItem(key)
