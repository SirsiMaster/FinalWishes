      ...
      {timeline ? (
        ...
        <div className="space-y-4">
          {Array.from({ length: cards }).map((_, i) => (
            <div key={i} className="flex items-start gap-5 p-5 bg-white rounded-2xl border border-slate-100">
        ...
        <div className={`grid grid-cols-1 ${gridCols} gap-6`}>
          {Array.from({ length: cards }).map((_, i) => (
            <div key={i} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 flex items-center gap-8">
