...
## [Unreleased]
### Fixed
- **Soul Log entries can now be edited (Phase 4 CRUD)** (`web/src/routes/estates.$estateId.soul-log.lazy.tsx`) — added an Edit affordance (pencil, beside delete) on each `EntryCard` → a lightweight title edit dialog → `updateDoc` on `estates/{id}/soul-log/{entryId}` (`title`, `updatedAt`) with toasts + saving state. Scoped to the title (safe for all entry types — no media re-record, no sealed-delivery side-effects). *(Rich text-content editing for text entries via the TipTap editor is a tracked follow-up.)*
